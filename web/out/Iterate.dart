// Auto-generated from Iterate.html.
// DO NOT EDIT.

library Iterate_html;

import 'dart:html' as autogenerated;
import 'dart:svg' as autogenerated_svg;
import 'package:web_ui/web_ui.dart' as autogenerated;
import 'package:web_ui/observe/observable.dart' as __observe;
import 'dart:html';
import 'package:web_ui/web_ui.dart';


// Original code


List<String> features = const <String>['lexical scope',
                                        'closures',
                                        'getters and setters',
                                        'isolates',
                                        'top-level functions',
                                        'optional static types',
                                        'one-line functions',
                                        'familiar syntax',
                                        'map and list literals',
                                        'generics'];

// TODO try a Set when it can be watched
List<String> userFavorites = toObservable(new List<String>());

addToFavorites(Event e) {
  InputElement checkbox = e.target;
  var fav = checkbox.value;
  
  if (checkbox.checked) {
    userFavorites.add(fav);
  } else if (!checkbox.checked) {
    var index = userFavorites.indexOf(fav);
    userFavorites.removeAt(index);
  }
  
  userFavorites.sort();
}

main() { }
// Additional generated code
void init_autogenerated() {
  var _root = autogenerated.document.body;
  final __html0 = new autogenerated.Element.html('<li>\n        <label></label>\n      </li>'), __html1 = new autogenerated.Element.html('<input type="checkbox">'), __html2 = new autogenerated.LIElement();
  var __e4, __e7;
  var __t = new autogenerated.Template(_root);
  __e4 = _root.nodes[7];
  __t.loopIterateAttr(__e4, () => features, ($list, $index, __t) {
    var feature = $list[$index];
    var __e0, __e2, __e3;
    __e3 = __html0.clone(true);
    __e2 = __e3.nodes[1];
    __e0 = __html1.clone(true);
    __t.listen(__e0.onClick, ($event) { addToFavorites($event); });
    __t.oneWayBind(() =>  feature , (e) { if (__e0.value != e) __e0.value = e; }, false, false);
    var __binding1 = __t.contentBind(() =>  feature , false);
    __e2.nodes.addAll([new autogenerated.Text('\n          '),
        __e0,
        new autogenerated.Text('\n          '),
        __binding1,
        new autogenerated.Text('\n        ')]);
  __t.addAll([new autogenerated.Text('\n      '),
      __e3,
      new autogenerated.Text('\n    ')]);
  });
  __e7 = _root.nodes[11];
  __t.loopIterateAttr(__e7, () => userFavorites, ($list, $index, __t) {
    var fav = $list[$index];
    var __e6;
    __e6 = __html2.clone(true);
    var __binding5 = __t.contentBind(() =>  fav , false);
    __e6.nodes.addAll([new autogenerated.Text('\n        '),
        __binding5,
        new autogenerated.Text('\n      ')]);
  __t.addAll([new autogenerated.Text('\n      '),
      __e6,
      new autogenerated.Text('\n    ')]);
  });
  __t.create();
  __t.insert();
}

//@ sourceMappingURL=Iterate.dart.map